...
$wt_short_title = 'SERVIZI';
$wt_full_title = 'SERVIZI';
...
if (is_object($form->fields['lettera'])) $form->fields['lettera']->help = 'Verrà utilizzata per identificare il servizio sulla mappa';
    ...
    new form_static_listbox($form, 'colore', 'COLORE', 'rosso,nero,blu,verde,grigio,arancione,viola,bianco,giallo', false, '');
if (is_object($form->fields['colore'])) $form->fields['colore']->help = 'Verrà utilizzato per identificare il servizio sulla mappa';
...
if (is_object($form->fields['latitudine'])) $form->fields['latitudine']->help = 'Coordinate in Gradi Decimali (Decimal Degrees - DD). In questo formato, sia la latitudine che la longitudine sono rappresentate come numeri decimali. Esempio: Latitudine: 41.40338, Longitudine: 2.17403.';

    
    new form_textbox($form, 'longitudine', 'LONGITUDINE', false, 30, 255, '');
if (is_object($form->fields['longitudine'])) $form->fields['longitudine']->help = 'Coordinate in Gradi Decimali (Decimal Degrees - DD). In questo formato, sia la latitudine che la longitudine sono rappresentate come numeri decimali. Esempio: Latitudine: 41.40338, Longitudine: 2.17403.';
...
$form->fields['provincia']->autosuggest_destfields = 'latitudine,longitudine,comune';
    ...
    $form->draw();
      ...
      $report_fields = 'id,cf_bool_is_special,cf_bool_is_selected,nome,ordine,lettera,colore,descrizione,latitudine,longitudine,comune,provincia,indirizzo,inizio,fine,operatoriasupporto,note,created,created_by,created_by_surname,created_by_name,last_upd,last_upd_by,last_upd_by_surname,last_upd_by_name,mod_num, uuid';
	  //$admin_report_fields = '';
      $default_fields = 'cf_bool_is_special,cf_bool_is_selected,nome,ordine,lettera,colore,descrizione,latitudine,longitudine,comune,provincia,indirizzo,inizio,fine,operatoriasupporto,note';
      ...
      if ($_CAMILA['page']->camila_exporting())
          $mapping = 'created=Data creazione#last_upd=Ultimo aggiornamento#last_upd_by=Utente ult. agg.#last_upd_src=Sorgente Ult. agg.#last_upd_by_name=Nome Utente ult. agg.#last_upd_by_surname=Cognome Utente ult. agg.#mod_num=Num. mod.#id=Cod. riga#created_by=Utente creaz.#created_src=Sorgente creaz.#created_by_surname=Cognome Utente creaz.#created_by_name=Nome Utente creaz.#cf_bool_is_special=contrassegnati come speciali#cf_bool_is_selected=selezionati#nome=NOME#ordine=ORDINE#lettera=LETTERA#colore=COLORE#descrizione=DESCRIZIONE#latitudine=LATITUDINE#longitudine=LONGITUDINE#comune=COMUNE#provincia=PROVINCIA#indirizzo=INDIRIZZO#inizio=INIZIO#fine=FINE#operatoriasupporto=OPERATORI A SUPPORTO#note=NOTE';
      else
          $mapping = 'created=Data creazione#last_upd=Ultimo aggiornamento#last_upd_by=Utente ult. agg.#last_upd_src=Sorgente Ult. agg.#last_upd_by_name=Nome Utente ult. agg.#last_upd_by_surname=Cognome Utente ult. agg.#mod_num=Num. mod.#id=Cod. riga#created_by=Utente creaz.#created_src=Sorgente creaz.#created_by_surname=Cognome Utente creaz.#created_by_name=Nome Utente creaz.#cf_bool_is_special=contrassegnati come speciali#cf_bool_is_selected=selezionati#nome=NOME#ordine=ORDINE#lettera=LETTERA#colore=COLORE#descrizione=DESCRIZIONE#latitudine=LATITUDINE#longitudine=LONGITUDINE#comune=COMUNE#provincia=PROVINCIA#indirizzo=INDIRIZZO#inizio=INIZIO#fine=FINE#operatoriasupporto=OPERATORI A SUPPORTO#note=NOTE';
