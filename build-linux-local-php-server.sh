...
build_php_from_source() {
	...
	wget "https://www.php.net/distributions/php-$PHP_VERSION.tar.xz" -O "php-$PHP_VERSION.tar.xz" ||
		{
			echo "Error downloading PHP sources"
			exit 1
		}
	...
	if command -v strip >/dev/null 2>&1; then
		echo ">>> Stripping PHP binary to reduce size..."
		strip "$PREFIX/bin/php" || true
		[ -f "$PREFIX/sbin/php-fpm" ] && strip "$PREFIX/sbin/php-fpm" || true
	fi

	echo ">>> Removing dev files (headers, static libs) from PHP prefix..."
	rm -rf "$PREFIX/include" || true
	find "$PREFIX/lib" -name '*.a' -delete || true
...
build_nginx_from_source() {
	...
	wget -O "$TARBALL" "$URL" ||
		{
			echo "Error downloading Nginx sources"
			exit 1
		}
...
# -----------------------------------------
wget -O "$SCRIPT_DIR/temp/src.zip" "https://github.com/linkingtechnologies/peichp/archive/refs/heads/main.zip" || {
